...
def ks_binary_events_for_pm_fallback(conn, sports: str) -> list[tuple[str, str, str, tuple[str, str]]]:
    ...
    for event_ticker, rows in grouped.items():
        ...
        if classification.universe not in PM_FALLBACK_UNIVERSES:
            continue
        event_date = sports_taxonomy.event_date_from_ks(event_ticker, str(rows[0]["close_time"] or ""), str(rows[0]["title"] or ""))
...
def collect_pm_repair_metadata(conn, args: argparse.Namespace) -> int:
    ...
    for slug in candidate_slugs:
        ...
        try:
            ...
            continue
        for event in events:
            market_db.upsert_pm_event(conn, event, tag_slug=pm_source_tag_for_slug(slug))
    ...
    for adapter in adapters:
        ...
        for tag_slug in category.tag_slugs:
            try:
                ...
                continue
            for event in events:
                market_db.upsert_pm_event(conn, event, tag_slug=tag_slug)
            count += len(events)
            conn.commit()
        for tag_id in category.tag_ids:
            try:
                ...
                continue
            for event in events:
                market_db.upsert_pm_event(conn, event, tag_slug=f"tag_id:{tag_id}", tag_id=str(tag_id))
            count += len(events)
            conn.commit()
    return count
...
def collect_ks_metadata(conn, adapters: list[SportAdapter], args: argparse.Namespace) -> int:
    ...
    for series_ticker in candidate_ks_series(adapters):
        ...
        for market in markets:
            market_db.upsert_ks_market(conn, series_ticker, market)
        count += len(markets)
        conn.commit()
    return count
