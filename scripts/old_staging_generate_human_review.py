...
def build_review(db_path: Path, from_date: str, out_dir: Path) -> dict[str, Any]:
    ...
    conn = old_market_db.connect(db_path)
    ...
    paired_csv = out_dir / "old_review_paired_events.csv"
    unmatched_csv = out_dir / "old_review_unmatched_events.csv"
    human_unmatched_csv = out_dir / "old_review_unmatched_human.csv"
    summary_csv = out_dir / "old_review_pairing_summary.csv"
    payload_json = out_dir / "old_human_review_payload.json"
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="data/old")
