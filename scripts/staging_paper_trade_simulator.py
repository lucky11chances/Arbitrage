...
def scaled_decimal(value: Any) -> Decimal | None:
    if value is None:
        return None
    return market_db.scaled_to_decimal(int(value))
...
def format_value(value: Any) -> str:
    ...
    if isinstance(value, Decimal):
        return market_db.format_decimal(value, places=6)
    return str(value)
...
def bbo_capped_fill(
    ...
    *,
    ...
    net_edge = ONE - gross_cost - fee(market_db.PM_FEE_RATE, pm_quote.price) - fee(market_db.KS_FEE_RATE, ks_quote.price)
...
def load_alert_candidates(conn: sqlite3.Connection, min_net_edge: Decimal, limit: int = 0) -> list[sqlite3.Row]:
    params: list[Any] = [market_db.to_scaled(min_net_edge)]
    limit_clause = ""
    if limit > 0:
        limit_clause = "LIMIT ?"
        params.append(limit)
...
def main() -> None:
    ...
    parser.add_argument("--min-net-edge", type=lambda value: parse_decimal(value, "--min-net-edge"), default=Decimal("0.02"))
    ...
    parser.add_argument("--output", default="data/staging/week3_paper_trades.csv")
    parser.add_argument("--summary-output", default="data/staging/week3_paper_trade_summary.csv")
    ...
    print(
        ...
        f"min_net_edge={market_db.format_decimal(args.min_net_edge)}; "
