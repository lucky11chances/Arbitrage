...
def build_polymarket_sports_inventory(limit: int) -> tuple[list[dict[str, Any]], list[str]]:
...
def pairing_status(category: sports_registry.SportsCategory) -> str:
...
def guess_market_type(outcomes: list[Any], market: dict[str, Any] | None) -> str:
    ...
    if len(outcomes) == 3 and any(universe_adapters.is_draw_or_tie(label) for label in labels):
