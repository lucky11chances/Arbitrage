...
class NbaGame:
    ...
    official_date: str
    game_date_utc: str
    away_team: old_nba_common.NbaTeam
    home_team: old_nba_common.NbaTeam
...
def pair_nba(pm_limit: int, ks_limit: int, from_date: str | None) -> tuple[list[core.PairedContract], list[str]]:
    ...
    ks_games = discover_ks_binary_events("nba", "KXNBAGAME", ks_limit, old_nba_common.parse_kalshi_event_date)
    schedule_dates = {game.event_date for game in pm_games}
    schedule_dates.update(game.event_date for game in ks_games)
    if not schedule_dates:
    ...
    for pm_game in pm_games:
        pm_team_a = old_nba_common.team_from_name(pm_game.outcome_a)
        pm_team_b = old_nba_common.team_from_name(pm_game.outcome_b)
        if pm_team_a is None or pm_team_b is None:
    ...
    pairs: list[core.PairedContract] = []
    for ks_game in ks_games:
        ks_teams = [old_nba_common.team_from_abbr(market_team_abbr(market.ticker)) for market in ks_game.markets]
        if any(team is None for team in ks_teams):
...
def fetch_nba_schedule_day(day: date) -> list[NbaGame]:
    ...
    for event in payload.get("events", []):
        ...
        for competitor in competitors:
            ...
            team = old_nba_common.team_from_name(str(raw_team.get("displayName") or "")) or old_nba_common.team_from_abbr(
