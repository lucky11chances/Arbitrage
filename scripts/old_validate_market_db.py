...
def validate_edge_formulas(conn) -> int:
    ...
    for row in conn.execute(
        ...
        pm_bid = old_market_db.scaled_to_decimal(row["pm_bid_scaled"])
        pm_ask = old_market_db.scaled_to_decimal(row["pm_ask_scaled"])
        ks_bid = old_market_db.scaled_to_decimal(row["ks_bid_scaled"])
        ks_ask = old_market_db.scaled_to_decimal(row["ks_ask_scaled"])
        require(
        ...
        expected_leg, expected_net, expected_cost = old_market_db.best_net_decimal(pm_bid, pm_ask, ks_bid, ks_ask)
        require(row["best_leg"] == expected_leg, f"edge {row['edge_snapshot_id']} best_leg mismatch")
        require(row["gross_cost_scaled"] == old_market_db.to_scaled(expected_cost), f"edge {row['edge_snapshot_id']} gross_cost mismatch")
        require(row["net_edge_scaled"] == old_market_db.to_scaled(expected_net), f"edge {row['edge_snapshot_id']} net_edge mismatch")

        pm_bid_size = old_market_db.scaled_to_decimal(row["pm_bid_size_scaled"])
        pm_ask_size = old_market_db.scaled_to_decimal(row["pm_ask_size_scaled"])
        ks_bid_size = old_market_db.scaled_to_decimal(row["ks_bid_size_scaled"])
        ks_ask_size = old_market_db.scaled_to_decimal(row["ks_ask_size_scaled"])
        ...
        require(
            row["best_leg_bbo_size_scaled"] == old_market_db.to_scaled(executable_size),
            f"edge {row['edge_snapshot_id']} executable size mismatch",
        )
        require(
            row["net_profit_at_bbo_scaled"] == old_market_db.to_scaled(expected_net * executable_size),
