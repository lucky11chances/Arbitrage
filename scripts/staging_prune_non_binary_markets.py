...
def parse_json_object(value: Any) -> dict[str, Any]:
    try:
        parsed = market_db.json.loads(str(value or "{}"))
    except ValueError:
        return {}
...
def outcome_keys(outcomes: list[str]) -> set[str]:
    return {sports_taxonomy.normalize_key(outcome) for outcome in outcomes}
...
def pm_moneyline_binary(title: str, question: str, outcomes: list[str], token_ids: list[str], market_raw: dict[str, Any]) -> tuple[bool, str]:
    ...
    if any(staging_pair_from_db.is_draw_or_tie(outcome) for outcome in outcomes):
    ...
    if staging_pair_from_db.DISALLOWED_PM_TERMS_RE.search(question) or staging_pair_from_db.DISALLOWED_PM_TERMS_RE.search(title):
    ...
    if keys == {"yes", "no"} and not staging_pair_from_db.MATCHUP_RE.search(text):
...
def classify_pm_market(row: sqlite3.Row) -> tuple[bool, str]:
    ...
    ok, reason = staging_pair_from_db.pm_is_main_binary_market(title, question, outcomes, token_ids)
    if ok:
...
def classify_ks_event(event_rows: list[sqlite3.Row]) -> tuple[bool, str]:
    ...
    if first.market_type not in staging_pair_from_db.SAFE_MARKET_TYPES:
    ...
    keys = outcome_keys(outcomes)
    if any(staging_pair_from_db.is_draw_or_tie(outcome) for outcome in outcomes) or keys in ({"over", "under"}, {"yes", "no"}):
...
def create_backup(db_path: Path, backup_dir: Path) -> Path:
    ...
    timestamp = market_db.utc_now().replace(":", "").replace("+", "_").replace(".", "_")
...
def execute_prune(conn: sqlite3.Connection, plan: PrunePlan) -> dict[str, int]:
    ...
    conn.execute("DELETE FROM data_quality_warnings WHERE stage IN ('normalize_market_db', 'pair_from_db', 'compute_edges', ?)", (STAGE,))
...
def main() -> None:
    ...
    parser.add_argument("--backup-dir", default="data/staging/backups")
    parser.add_argument("--no-backup", action="store_true")
    parser.add_argument("--vacuum", action="store_true")
    args = parser.parse_args()

    db_path = Path(args.db)
    with market_db.connect(db_path) as conn:
