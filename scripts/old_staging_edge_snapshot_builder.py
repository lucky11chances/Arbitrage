...
def nearest_complete_observation(conn, venue: str, instrument_id: str, target_ts: str, max_skew_seconds: float):
    target = old_market_db.parse_iso_ts(target_ts)
...
def insert_edge_snapshot(
    ...
    *,
    ...
    return insert_edge_snapshot_detailed(
        conn,
        pair,
        pm_obs,
        ks_obs,
...
def insert_edge_snapshot_detailed(
    conn,
    pair,
    pm_obs,
    ks_obs,
    *,
    ...
    if pm_obs is None or ks_obs is None:
        return EdgeInsertOutcome(InsertStats(missing=1), status="error", reason="missing_observation")
    if str(pm_obs["status"]) != "ok" or str(ks_obs["status"]) != "ok":
        reasons = []
        if str(pm_obs["status"]) != "ok":
            reasons.append(f"pm_error:{pm_obs['error_message'] or 'unknown'}")
        if str(ks_obs["status"]) != "ok":
            reasons.append(f"ks_error:{ks_obs['error_message'] or 'unknown'}")
        return EdgeInsertOutcome(InsertStats(missing=1), status="error", reason=";".join(reasons))
    ...
    if dedupe and snapshot_exists(conn, pair_id, pm_observation_id, ks_observation_id):
        return EdgeInsertOutcome(InsertStats(duplicate=1), status="ok", reason="duplicate_edge_snapshot")

    pm_ts = str(pm_obs["collected_ts_utc"])
    ks_ts = str(ks_obs["collected_ts_utc"])
    snapshot_ts = max(old_market_db.parse_iso_ts(pm_ts), old_market_db.parse_iso_ts(ks_ts)).isoformat()
    book_age = max(
        (old_market_db.parse_iso_ts(snapshot_ts) - old_market_db.parse_iso_ts(pm_ts)).total_seconds(),
        (old_market_db.parse_iso_ts(snapshot_ts) - old_market_db.parse_iso_ts(ks_ts)).total_seconds(),
    )
    skew = old_market_db.seconds_between(pm_ts, ks_ts)
    if book_age > max_age_seconds or skew > max_skew_seconds:
        return EdgeInsertOutcome(
            InsertStats(stale_or_skewed=1),
            status="stale_or_skewed",
            reason=f"book_age={book_age:.3f}s skew={skew:.3f}s",
            book_age_seconds=book_age,
            snapshot_skew_seconds=skew,
        )

    edge = old_market_db.compute_edge(pm_obs, ks_obs)
    if edge.net_edge is None:
        return EdgeInsertOutcome(
            InsertStats(incomplete_edge=1),
            status="missing_depth",
            reason="missing_bbo_price_or_size",
            book_age_seconds=book_age,
            snapshot_skew_seconds=skew,
        )
    if edge.best_leg_bbo_size is None or edge.best_leg_bbo_size <= Decimal("0"):
        return EdgeInsertOutcome(
            InsertStats(non_executable=1),
            status="non_executable",
            reason=f"non_executable_bbo_size={edge.best_leg_bbo_size}",
            book_age_seconds=book_age,
            snapshot_skew_seconds=skew,
        )
    ...
    cursor = conn.execute(
        ...
        INSERT INTO edge_snapshots (
            paired_contract_id, snapshot_window_id, ts_utc, pm_observation_id, ks_observation_id,
        ...
        (
            ...
            edge.best_leg,
            old_market_db.to_scaled(edge.gross_cost),
            old_market_db.to_scaled(edge.net_edge),
            old_market_db.to_scaled(edge.best_leg_bbo_size),
            old_market_db.to_scaled(edge.net_profit_at_bbo),
            "ALERT" if alert else "",
    ...
    return EdgeInsertOutcome(
        ...
        edge_snapshot_id=int(cursor.lastrowid),
        status="ok",
        reason="",
        book_age_seconds=book_age,
        snapshot_skew_seconds=skew,
...
def build_latest_once(conn, pairs, args) -> InsertStats:
    ...
    for pair in pairs:
        ...
        if pm_obs is None or ks_obs is None:
            ...
            continue
        current_age = max(
            old_market_db.age_seconds(str(pm_obs["collected_ts_utc"]), now_ts),
            old_market_db.age_seconds(str(ks_obs["collected_ts_utc"]), now_ts),
        )
...
def main() -> None:
    ...
    latest.add_argument("--interval", type=float, default=10.0)
