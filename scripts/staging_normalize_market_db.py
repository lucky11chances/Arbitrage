...
def normalize_db(conn: sqlite3.Connection, sports: str = "all") -> dict[str, int]:
    ...
    market_db.reset_normalized_contracts(conn)
    conn.execute("DELETE FROM data_quality_warnings WHERE stage = 'normalize_market_db'")
...
def normalize_pm(conn: sqlite3.Connection, sports: str) -> int:
    ...
    for row in pm_token_rows(conn):
        ...
        classification = sports_taxonomy.classify_pm_event(
            str(row["event_slug"] or ""),
        ...
        event_date = sports_taxonomy.event_date_from_pm(
            str(row["event_slug"] or ""),
            str(row["start_date"] or ""),
            str(row["event_title"] or ""),
        )
        entity_key = sports_taxonomy.entity_key(
        ...
        candidate = NormalizedCandidate(
            ...
            outcome_key=entity_key or sports_taxonomy.normalize_key(str(row["outcome_name"] or "")),
        ...
        market_db.upsert_normalized_candidate(conn, candidate, universe=classification.universe)
        count += 1
    return count
...
def normalize_ks(conn: sqlite3.Connection, sports: str) -> int:
    ...
    for row in ks_market_rows(conn):
        classification = sports_taxonomy.classify_ks_market(
        ...
        candidate = NormalizedCandidate(
            ...
            entity_key=entity_key,
            outcome_key=entity_key or sports_taxonomy.normalize_key(str(row["yes_sub_title"] or "")),
